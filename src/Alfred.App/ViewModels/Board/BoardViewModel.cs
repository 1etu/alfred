...
public sealed class BoardViewModel : PageViewModel
{
    private readonly Vault _vault;
    private int _nextTint;

    public BoardViewModel(Vault vault)
        : base(LocalizationService.Text(LocalizationKeys.NavFreeBoard), "FreeBoardIcon")
    {
        _vault = vault;
        Columns =
        [
            new ColumnModel(BoardColumn.Backlog, LocalizationService.Text(LocalizationKeys.BoardBacklog), this),
            new ColumnModel(BoardColumn.Doing, LocalizationService.Text(LocalizationKeys.BoardDoing), this),
            new ColumnModel(BoardColumn.Done, LocalizationService.Text(LocalizationKeys.BoardDone), this),
        ];
        Refresh();
    }
    ...
    public sealed class CardRow
    {
        ...
        public string TintKey => TintKeys[Math.Clamp(Card.Tint, 0, TintKeys.Length - 1)];
